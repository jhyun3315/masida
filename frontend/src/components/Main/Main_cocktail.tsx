        ...
        <div className={ style.mainCocktail_content}>
          <div className={ style.mainCocktail_recommend}>
            <div className={ style.mainCocktail_recommend_img}>
              <Image src={cocktailInfo.cocktail_img} loader={({ src, width, quality }: ImageLoaderProps) => imgLoader({ src, width, quality })} alt="image" width={300} height={300} />
            </div>
            <div className={style.mainCocktail_recommend_detail }>
              <h2>{cocktailInfo.cocktail_name_ko}({cocktailInfo.cocktail_name_en})</h2>
              <p>{cocktailInfo.cocktail_content} <br /> <Image src="/assets/icons/ratingICON.png" loader={({ src, width, quality }: ImageLoaderProps) => imgLoader({ src, width, quality })} alt="image" width={10} height={10}></Image>{cocktailInfo.cocktail_rating}({cocktailInfo.cocktail_comments})</p>
            </div> 
         </div>
              ...
              <Slider {...settings}>
                {cocktailList.map((key) => (
                  ...
                  <div className={style.cocktailTopLike_Img}>
                      <Image src={key.cocktail_img} loader={({ src, width, quality }: ImageLoaderProps) => imgLoader({ src, width, quality })}  alt="image" width={200} height={200} />
                  </div>
                  <div className={style.cocktailTopLike_desc}>
                      <p>{ key.cocktail_name_ko}</p>
                      <p>({ key.cocktail_name_en})</p>
                      <p><Image src="/assets/icons/ratingICON.png" loader={({ src, width, quality }: ImageLoaderProps) => imgLoader({ src, width, quality })} alt="image" width={10} height={10}></Image>{key.cocktail_rating}({key.cocktail_comments })</p>
                  </div>
                </div>
                ))}
               </Slider>
