...
public class WorldCupImpl implements WorldCupService {
    ...
    public ArrayList<CocktailWorldCupDetail> getWorldCupCocktails() {
        ...
        for (int i = 0; i < 16; i++) {
            Cocktail cocktail = cocktails.get(i); // 칵테일 가져오기
            CocktailWorldCupDetail worldCocktail = new CocktailWorldCupDetail(); // 월드컵에 넣을 칵테일
            worldCocktail.setCocktailId(cocktail.getId()); // 칵테일 id 삽입
            worldCocktail.setCocktailNameKo(cocktail.getCocktailNameKo()); // 칵테일 한글 이름 삽입
            worldCocktail.setCocktailNameEn(cocktail.getCocktailNameEn()); // 칵테일 영어 이름 삽입
            worldCocktail.setCocktailImg(cocktail.getCocktailImg()); // 칵테일 이미지 삽입
            worldCocktail.setCocktailContent(cocktail.getCocktailContent()); // 칵테일 내용 삽입
            worldCocktail.setIngredient(new ArrayList<IngredientInfo>()); // 칵테일 재료를 저장하는 객체 삽입
            String cocktaildifficulty = (int)cocktail.getCocktailDifficulty() == 1 ? "하" :
                    (int)cocktail.getCocktailDifficulty() == 2 ? "중" : "상"; // 칵테일 난이도
            worldCocktail.setCocktailDifficulty(cocktaildifficulty); // 칵테일 난이도 삽입
            worldCocktail.setCocktailRating(Math.round(cocktail.getCocktailRating() * 10.0) / 10.0); // 칵테일 평점 삽입
            List<Like> likes = likeRepository.findAllByCocktailAndLikeDeleted(cocktail, false); // 좋아요 가져오기
            worldCocktail.setCocktailLikes(likes.size()); // 좋아요 개수 삽입
            List<Comment> comments = commentRepository.findAllByCocktailAndCommentDeleted(cocktail, false); // 댓글 가져오기
            worldCocktail.setCocktailComments(comments.size()); // 댓글 개수 삽입
            ...
            for (CocktailIngredient cocktailIngredient: cocktailIngredients) { // 칵테일 재료
                ...
                worldCocktail.getIngredient().add(ingredientInfo); // 재료 삽입
            }
            cocktailWorldCupDetails.add(worldCocktail); // 월드컵 칵테일 추가
        }
